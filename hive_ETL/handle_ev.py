...
#解析dmalog
def parse_dmalog(sql_map):
    DMALOG_FILEDS= ['city','atype','business','ca_name','ca_source','ca_from','ca_kw','dialog_button','label','has_profile']
    if 'request_uri_dmalog' in sql_map.keys():
        dmalog = str(sql_map['request_uri_dmalog'])
        handle_array = DMALOG_FILEDS
        field_array  = re.split("@",dmalog)
        for field in field_array:
            result_array  = re.split("=",field)
            if len(result_array) == 2:
                key =  result_array[0]
                value = result_array[1]
                if key == 'dmalog':
                    sql_map['request_uri_dmalog_page'] = value
                else:
                    if key in DMALOG_FILEDS:
                        handle_array.remove(key)
                        sql_map['request_uri_dmalog_'+key] = value
        for handle_field in handle_array:
            sql_map['request_uri_dmalog_'+handle_field] = ''
    return sql_map
